...
function getPreferredImageUri(card: ScryfallSearchResponse['data'][number]): { url: string; extension: 'png' | 'jpg' } | null {
...
function getPublicImageUrl(key: string): string {
  ...
  if (!s3Bucket || !awsRegion) throw new Error('AWS_S3_BUCKET e AWS_REGION são obrigatórios para gerar URL do bucket');
  return `https://${s3Bucket}.s3.${awsRegion}.amazonaws.com/${key}`;
}

async function downloadImageLocally(url: string, scryfallId: string, extension: 'png' | 'jpg'): Promise<string> {
...
async function uploadImageToS3(url: string, scryfallId: string, extension: 'png' | 'jpg'): Promise<string> {
  if (!s3Client || !s3Bucket) {
    throw new Error('S3 não configurado');
  }

  const key = `cards/${scryfallId}.${extension}`;
  const response = await axios<ArrayBuffer>({ url, responseType: 'arraybuffer' });
...
async function storeImage(url: string, scryfallId: string, extension: 'png' | 'jpg'): Promise<string> {
  if (useS3) {
...
async function fetchAndSeed(): Promise<void> {
  ...
  console.log(useS3 ? "☁️ Upload configurado para S3/CloudFront" : "💾 Upload local em public/cards");
      ...
      try {
        ...
        for (const card of data.data) {
          ...
          if (existingCard?.imagePath) {
            ...
            imagePath = await storeImage(preferredImage.url, card.id, preferredImage.extension);
          }
