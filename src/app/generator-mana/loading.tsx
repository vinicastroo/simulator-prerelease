...
function SectionSkeleton({ rows = 5, delay = 0 }: { rows?: number; delay?: number }) {
  return (
    <div className="overflow-hidden rounded-[1.5rem] border border-white/10 bg-black/20 backdrop-blur-xl">
      <div className="border-b border-white/8 px-5 py-3">
        <div className="h-2.5 w-32 rounded-full animate-pulse bg-white/[0.06]" style={{ animationDelay: `${delay}ms` }} />
        <div className="mt-1.5 h-2 w-48 rounded-full animate-pulse bg-white/[0.03]" style={{ animationDelay: `${delay + 40}ms` }} />
      </div>
      <div className="divide-y divide-white/[0.05]">
        {Array.from({ length: rows }).map((_, i) => (
          <div key={i.toString()} className="flex items-center justify-between px-5 py-3">
            <div className="flex items-center gap-2.5">
              <div className="h-5 w-5 rounded-full animate-pulse bg-white/[0.06]" style={{ animationDelay: `${delay + i * 30}ms` }} />
              <div className="h-3.5 w-20 rounded-full animate-pulse bg-white/[0.05]" style={{ animationDelay: `${delay + i * 30 + 20}ms` }} />
            </div>
            <div className="flex items-center gap-1.5">
              <div className="h-7 w-7 rounded-lg animate-pulse bg-white/[0.04]" style={{ animationDelay: `${delay + i * 30 + 40}ms` }} />
              <div className="h-4 w-5 rounded animate-pulse bg-white/[0.05]" style={{ animationDelay: `${delay + i * 30 + 50}ms` }} />
              <div className="h-7 w-7 rounded-lg animate-pulse bg-white/[0.04]" style={{ animationDelay: `${delay + i * 30 + 60}ms` }} />
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="relative mx-auto flex max-w-lg flex-col px-4 py-4 sm:px-6 sm:py-6">
        <header className="shrink-0 px-1 pb-6">
          ...
          <div className="mt-6 space-y-2">
            ...
            <div className="h-10 w-48 rounded-full animate-pulse bg-white/[0.08]" style={{ animationDelay: "60ms" }} />
            <div className="h-3 w-72 rounded-full animate-pulse bg-white/[0.03]" style={{ animationDelay: "120ms" }} />
          </div>
        </header>
        ...
        <div className="space-y-4">
          ...
          <div className="h-11 w-full rounded-full animate-pulse bg-white/[0.08]" style={{ animationDelay: "200ms" }} />
        </div>
      </div>
    </main>
  );
